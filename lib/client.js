window.__ModuleLoader__.load({id:'dsh-zhixing',factory:(require)=>{
  ...
  function apply(ctx){
    ...
    ctx.slots.inject('sidebar.footer.action',()=>ctx.slots.register({name:'sidebar.footer.action',id:'zhixing-entry',order:10,inject:()=>({openLearning:async()=>{let current=ctx.sessions.list.getSnapshot().current;if(!current){const w=ctx.workspaces.list.getSnapshot().items?.[0];if(!w)return;current=await ctx.sessions.create({workspaceId:w.workspaceId,sessionId:crypto.randomUUID()});}ctx.sessions.open(current);
      try{const key=`dsh.conversation.${current}`;const raw=localStorage.getItem(key);const state=raw?JSON.parse(raw):{};localStorage.setItem(key,JSON.stringify({...state,view:'zhixing'}));}catch{}
      // dsh 0.1.x does not expose view selection; activate the registered tab in-place.
      const activate=()=>{const nodes=[...document.querySelectorAll('button,[role="tab"]')];const tab=nodes.find(n=>n.textContent?.trim()==='学习'||n.getAttribute('aria-label')==='学习');if(tab){tab.click();return true;}return false;};
      if(!activate()){let tries=0;const timer=setInterval(()=>{if(activate()||++tries>20)clearInterval(timer);},100);}
    }})},LearningEntry));
  }
