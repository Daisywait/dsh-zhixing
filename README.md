...
取得 `dsh-zhixing-0.3.3.tgz` 安装包（或在源码目录运行 `npm pack`）后运行：

```sh
dsh plugin --profile web add ./dsh-zhixing-0.3.3.tgz
```
...
侧栏底部的“知行学习”入口在欢迎页也可用，点击打开内嵌面板。会话内原有“学习”标签继续保留。面板中的“全部学习主题”可直接切换主题；“继续学习”先打开目标会话再切换视图，学习请求会进入该会话草稿。新建学习会话需要当前会话已选择工作区；没有工作区时先在 dsh 欢迎页选择，已有学习主题仍可浏览。

1. 在“学习”页输入想弄明白的问题，点“从这里开始”。
2. 默认新建独立学习会话，请求进入新会话草稿，你确认发送后开始。也可以选择关联已有会话。
3. 回到学习页看下一步；点“我的理解”展开靶图和作答记录。

“继续学习”回到主题的主要学习会话，准备草稿，不自动发送。也可以点“先讲给我听”“换个例子”或“试一道新题”。已记录的例子直接呈现具体情况、推测过程和结果，可切换比较，再对照规则。“这一步没看懂”“例子或解释有问题”将当前材料带回会话，请求补充或核查，不计作答。已有草稿保留。新会话继承当前工作目录，模型由 dsh 的新会话默认配置决定。

新学习轮可在“为什么接下来学这个”中查看材料选择的依据，区分暂定判断、自述和真实作答。旧档案没有这部分时仍可正常学习，后续由助手逐步记录，不会凭空补写历史成绩。

完全陌生的主题先得到适配的讲解和实例，已有基础时可以直接验证。助手依据反馈选择下一份材料，不采用固定层级闯关。教学例子可以展示答案，验证题等待真实回答后再核对。[方法依据与产品约定](docs/learning-design.md)分别记录。
