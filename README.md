...
嵌入 DeepSeek Harness 的学习助手。用一个问题开始，在原来的聊天框回答；助手记录你的理解和真实作答，学习页显示下一步。需要复盘时展开渐构靶图，查看概念、规则与实例的对应。
...
下载发布页的 `dsh-zhixing-0.2.2.tgz` 后运行：

```sh
dsh plugin --profile web add ./dsh-zhixing-0.2.2.tgz
```
...
2. 请求进入当前聊天草稿，你确认发送后开始。每次只回答一道题。
3. 回到学习页看下一步；点“我的理解”展开靶图和作答记录。

“继续练一题”也会填入当前聊天草稿，不自动发送。已有草稿保留。模型草案和说明实例不是学习成绩；有提示答对与独立答对分开记录。
